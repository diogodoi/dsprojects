...
st.subheader('Predição Multi Modal')
...
def load_data():
    ...
    df['sqft_living'] = df.sqft_living.map(lambda p: p/10.764)
    return df

df = load_data()


st.sidebar.subheader('Menu de seleção')
params = {
    'bedrooms':st.sidebar.selectbox('Quartos',(1,2,3,4,5)),
...
test_size = st.sidebar.slider('Escolha o tamanho do teste',0.05,0.5,0.25,step=0.05)
...
def show_ML():
    ...
    df_models
    st.write('**O Gráfico abaixo mostra o erro médio quadrático de cada modelo**')
    st.bar_chart(df_models['RMSE_Price'])
    
btn = st.sidebar.button("Prever")
if btn:
    run_data()
else:
    pass

if st.sidebar.button('Visualizar JSON'):
    df_models = get_models()[0][0]
    st.json(map_df(df).to_json())
    
if st.sidebar.button('Fechar JSON'):
    run_data()
    
st.sidebar.subheader('Informações adicionais')
if st.sidebar.checkbox('Visualizar modelo ML'):
    run_data()
    df_models = get_models()[1]
    df_models
    st.write('**Os Gráficos mostram o erro médio quadrático de todos os modelos usados**')
    st.bar_chart(df_models['RMSE_Price'])

if st.sidebar.checkbox('Vizualizar todos os dados'):
    df
